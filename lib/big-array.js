 ...
 *
 * @param options
 * @param or @options.dir directory for array data store
 * @param or @options.name the name of the array, will be appended as last part of the array directory
...
function BigArray(dir, name, dataPageSize, maxDataSize) {
    ...
    this.arrayDirectory = Path.join(dir, name);
...
proto.eachSync = function eachSync(iteratorFn) {
    ...
    var index = this.arrayTailIndex;
    var size = this.size();
    for (var i = index; i.lt(size); i = i.add(1)) {
        iteratorFn(this.get(i), i);
    }
};
...
proto.each = function each(iteratorFnAsync, callback) {
    ...
    var size = this.size();
    var innerArray = this;
    var index = this.arrayTailIndex.sub(1);
